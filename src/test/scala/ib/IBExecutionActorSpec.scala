...
with WordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "execution book" must{
    ...
    "publish position update for single sell execution" in{
      ...
      source.expectMsg(PublishableEvent("position/test1/1",  IBPosition("test1",contract1, -2, -3, 0)))
    }
    "publish position update for single buy execution" in{
      ...
      source.expectMsg(PublishableEvent("position/test2/2",  IBPosition("test2",contract2, 2, 3, 0)))
    }
    ...
    "publish position update for multiple buy execution" in{
      ...
      executor ! event
      source.expectMsg(PublishableEvent("position/test3/3",  IBPosition("test3",contract3, 2, 3, 0)))

      val event1 = generateExecution(contract3,"BOT", 2, 2, 1.5, 1.5)
      executor ! event1
      source.expectMsg(PublishableEvent("position/test3/3",  IBPosition("test3",contract3, 4, 6, 0)))
    ...
    "publish current position after re-start" in{
      ...
      val event = generateExecution(contract3,"BOT", 2, 2, 1.5, 1.5)
      executor ! event
      source.expectMsg(PublishableEvent("position/test4/3",  IBPosition("test4",contract3, 2, 3, 0)))

      val event1 = generateExecution(contract3,"BOT", 2, 2, 1.5, 1.5)
      executor ! event1
      source.expectMsg(PublishableEvent("position/test4/3",  IBPosition("test4",contract3, 4, 6, 0)))
    ...
    "calculate realized pnl for multiple executions" in{
      ...
      execs.foldRight(IBPosition("service1",contract3, 0, 0, 0))((e, p) => p.update(e)) should be(IBPosition("service1",contract3, 2, 3, 0.20000000000000018))
    }
